...
//! (vendored, scalar-only) complex FFT engine. See `dd_fft::vendor` module docs for why this is
...
use super::rustfft::{Complex, Fft, FftDirection, FftNum, FftPlannerScalar};
...
fn compute_twiddle<T: FftNum>(index: usize, fft_len: usize) -> Complex<T> {
    super::rustfft::twiddles::compute_twiddle(index, fft_len, FftDirection::Forward)
}

pub struct RealToComplexOdd<T> {
    length: usize,
    fft: std::sync::Arc<dyn Fft<T>>,
    scratch_len: usize,
}
...
impl<T: FftNum> RealFftPlanner<T> {
    ...
    pub fn new() -> Self {
        let planner = FftPlannerScalar::<T>::new();
        Self {
...
impl<T: FftNum> RealToComplexOdd<T> {
    ...
    pub fn new(length: usize, fft_planner: &mut FftPlannerScalar<T>) -> Self {
        if length % 2 == 0 {
