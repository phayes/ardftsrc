...
/// specifically to compute twiddles for [`Dd`], our double-double type. So instead of a blanket
/// impl, `twiddle()` is a required method implemented per-type: `f32`/`f64` keep upstream's
/// f64-intermediate behavior (kept only so this vendored engine can be cross-checked against the
/// real `realfft`/`rustfft` output at matching precision), and `Dd` computes its twiddle factors
/// natively in double-double precision.
...
impl FftNum for Dd {
    fn twiddle(numerator: usize, denominator: usize, direction: FftDirection) -> Complex<Self> {
        Dd::twiddle_factor(numerator, denominator, direction)
    }
}
    ...
    ($struct_name:ident, $len_fn:expr) => {
        impl<T: FftNum> Fft<T> for $struct_name<T> {
            fn process_immutable_with_scratch(
                ...
                crate::dd_fft::vendor::rustfft::fft_helper::fft_helper_immut(
                    input,
                    output,
                    scratch,
                    self.len(),
            ...
            fn process_outofplace_with_scratch(
                ...
                crate::dd_fft::vendor::rustfft::fft_helper::fft_helper_outofplace(
                    input,
                    output,
                    scratch,
                    self.len(),
            ...
            fn process_with_scratch(&self, buffer: &mut [Complex<T>], scratch: &mut [Complex<T>]) {
                crate::dd_fft::vendor::rustfft::fft_helper::fft_helper_inplace(
                    buffer,
                    scratch,
                    self.len(),
