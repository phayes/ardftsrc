...
//! `dd_fft::vendor` module docs for why this is vendored rather than depended on.
//!
...
//! around them (`FftPlannerScalar` is used directly instead) -- double-double arithmetic isn't a
...
//! covered by `dd_fft::real`'s tests, which exercise this engine through the real-FFT layer.
//!
...
//!   for `Dd` (genuinely double-double). See `common.rs` for the full rationale.
